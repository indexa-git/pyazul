...
A unique `secure_id` is generated by PyAzul during initiation and is automatically appended to your `TermUrl` and `MethodNotificationUrl` to help track the session.

### Application-Side State Management (Crucial for 3DS)

While PyAzul's `SecureService` uses an internal in-memory session to link `secure_id` to transaction data (like `azul_order_id`), this is **not persistent and is instance-specific**.
**For production environments, especially those with multiple server instances or requiring data persistence across application restarts, your application MUST implement its own robust session management solution** (e.g., using Redis, a database, or your web framework's session handling). This system will store and retrieve necessary data (e.g., your internal order details, `azul_order_id` if obtained early, the `original_term_url`) using the `secure_id` as the key across the different callback steps.

The FastAPI examples below use a simple dictionary (`app_level_session_store`) for illustrative purposes only. **Do not use this in-memory dictionary for production.**
...
async def initiate_3ds_payment(request_data: dict): # Process incoming payment data
    original_term_url = request_data.get("threeDSAuth", {}).get("TermUrl", "") # Store for later use in challenge form
    ...
    try:
        ...
        if secure_id:
            ...
            # print(f"PyAzul internal session for {secure_id}: {pyazul_internal_session}")
        ...
        if response.get("redirect"):
            ...
            # Approved directly by Azul (e.g., frictionless flow)
...
async def capture_3ds_method_callback(secure_id: str = Query(...)): # secure_id is appended by PyAzul
    try:
        ...
        # azul_order_id is required by PyAzul's secure_3ds_method.
        # Ensure it was stored or retrieve if necessary.
        if not azul_order_id:
            ...
            if pyazul_session and pyazul_session.get("azul_order_id"):
                ...
                return JSONResponse(content={"error": "Azul Order ID not retrievable for secure_id"}, status_code=400)

        if not original_term_url:
            return JSONResponse(content={"error": "Original TermUrl not found in app session"}, status_code=400)
        ...
        if result.get("ResponseMessage") == "3D_SECURE_CHALLENGE":
            ...
            form_html = azul.create_challenge_form(
                ...
                original_term_url, # Your original TermUrl (PyAzul added secure_id)
...
# --- Payment Page ---
# html_form_string = azul.create_payment_page(data_dict_or_model_dump)
...
# response_dict = await azul.secure_3ds_method(azul_order_id="...", method_notification_status="...")
# response_dict = await azul.secure_challenge(secure_id="...", cres="...")
