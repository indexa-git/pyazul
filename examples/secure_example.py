...
async def capture_3ds(
    ...
    try:
        ...
        if not session:
            ...
            return templates.TemplateResponse(
                "error.html", {"request": request, "error": "Invalid session"}
            )
...
async def post_3ds(
    ...
    if not session_id:
        return templates.TemplateResponse(
            "error.html",
            {"request": request, "error": "No session identifier provided"},
        )

    if not cres:
        return templates.TemplateResponse(
            "error.html",
            {"request": request, "error": "No challenge response provided"},
        )
